			    ...
			    <div class="content clearfix">
			    	<div class="clearfix" :style="{height: boxHeight + 'px'}">
			    		...
			    		<div class="right" :style="{height: boxHeight - 25 + 'px'}">
						    ...
						    <div class="listLoading" v-if="listLoading2">
							  	<img src="../../assets/img/tableLoading.gif" width="20"/>
							  	<div>拼命加载中</div>
							</div>
...
export default{
	...
	methods: {
		...
		getSignedDetail(){
		    ...
		    .then(res => {
		        ...
		        if (res.data.Code == 0) {
		        	...
		        	_this.signedInfo = res.data.Data;
		...
		sendFile(){
	        ...
	        this.$axios({
		        method: "post",
		        url: _this.$link + "api/Contract/SignContract",
		        headers: { "access-token": _this.userInfo.Token },
		        data: {
		          timestamp: "1",
		          SecretId: "3",
	        ...
	        .then(res => {
	         // console.log(res.data,'000');
	          ...
	          if (res.data.Code == 0) {
	          	this.$router.push({ path: "/filesParent/files/1"});
	          }else if (res.data.Code == 400) {
		          _this.$router.push({ path: "/login" });
		      } else {
		...
		saveToDraft(){
	        ...
	        .then(res => {
	            ...
	            if(res.data.Code == "0"){
	          	   this.$router.push({ path: "/filesParent/files/7"});
		        }else{
		      	   alert(res.data.Message);
		       }
	      ...
	      this.$axios({
	        ...
	        .then(res => {
	         // console.log(res.data,'000');
	          ...
	          if (res.data.Code == 0) {
	          	//只选取前面3个
	             _this.signList = (res.data.Data).slice(0,3);
