	...
	<div id="addFile">
		...
		<div class="addFile">
  		...
  		<alert-dialog :is-show="isShowAlert" :alertCont="alertContent" 
  			@on-close="closeAlert" type="default"></alert-dialog>
...
export default{
	...
	methods: {
		...
		/*moreImg(e){
			var files = e.target.files[0];
			this.fileInfo.push(files);  //保存到文件信息
			if(files.type.indexOf('image') == 0){
				var localUrl = this.getObjectURL(files);
				this.imgUrl.push(localUrl);
			}
		},*/
		...
		getDetailInfo(){
		    ...
		    .then(res => {
		        if (res.data.Code == 0) {
		        	...
		        	_this.fileInfo = res.data.Data;
		        	_this.fileSize = res.data.Data.FileSize;
		        	_this.fileName = res.data.Data.ContractName;
					_this.imgUrls = res.data.Data.Images;
					_this.contractID = res.data.Data.ContractID;
		...
		savaFile(){
			...
			this.$axios({
				method: "post",
				url: _this.$link + "api/Contract/Modify",
				headers: {
		...
		WordToPdf(fileInfo) {
			...
			.then(res => {
				...
				if(res.data.Code == "0") {
					_this.fileInfo = res.data.Data;
					_this.imgUrls = res.data.Data.ImageUrls;
					_this.contractID = res.data.Data.ID;
				} else if(res.data.Code == "400"){
		        	_this.$router.push({ path: "/login" });
		        }else {
					_this.isShowAlert = true;
